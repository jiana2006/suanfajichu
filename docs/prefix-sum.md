...
差分的定义是，对于一个数组 nums，差分数组 diff 的第 i 个元素表示原数组 nums 中的第 i 个元素与第 i-1 个元素的差。
...
```
arr  1        3         7            5              2
diff 1   (3-1)2    (7-2)5       (5-5)0         (2-0)0

```
差分的一个用处当我们需要对一个区间内的数相减在求和的时候。

我们就可以用差分数组来实现。

比如说要对L-R区间的数减去x然后对L-R区间求和。

此时我们只需要让对应的差分数组diff[L]-=x;diff[R+1]+=x;我们只需要该这两个数组即可。

然后我们就可以用前缀和公式来求L-R区间的和。
